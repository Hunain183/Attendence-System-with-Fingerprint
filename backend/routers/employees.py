...
async def debug_raw_request(request: Request):
    ...
    try:
        ...
        print(f"DEBUG JSON: {json.dumps(json_body, indent=2)}")
        return {"raw": body.decode(), "json": json_body}
    except Exception as e:
        return {"raw": body.decode(), "error": str(e)}
...
async def create_employee(
    ...
    """
    print(f"DEBUG - Employee data received: {employee_data}")
    try:
